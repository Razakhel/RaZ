...
RUN sudo apt-get install -y libglew-dev libx11-dev libxcursor-dev libxrandr-dev libxinerama-dev
# CMake, GCC & Clang to build RaZ
RUN sudo apt-get install -y cmake gcc clang
# Doxygen & Dot to generate the documentation
RUN sudo apt-get install -y doxygen python-pydot python-pydot-ng
...
RUN sudo chmod +x ./fbx20190_fbxsdk_linux
