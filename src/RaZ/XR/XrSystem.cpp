...
void XrSystem::recoverViewConfigurations() {
  ...
  m_viewConfigs.resize(viewConfigCount);
  checkLog(xrEnumerateViewConfigurations(m_context.m_instance, m_context.m_systemId, viewConfigCount, &viewConfigCount, m_viewConfigs.data()),
           "Failed to enumerate view configurations",
           m_context.m_instance);

  for (const XrViewConfigurationType viewConfiguration : { XR_VIEW_CONFIGURATION_TYPE_PRIMARY_STEREO, XR_VIEW_CONFIGURATION_TYPE_PRIMARY_MONO }) {
    if (std::find(m_viewConfigs.cbegin(), m_viewConfigs.cend(), viewConfiguration) == m_viewConfigs.cend())
      continue;
  ...
  checkLog(xrEnumerateViewConfigurationViews(m_context.m_instance, m_context.m_systemId, m_viewConfig, 0, &viewConfigViewCount, nullptr),
           "Failed to get view configuration view count",
           m_context.m_instance);
  m_viewConfigViews.resize(viewConfigViewCount, { XR_TYPE_VIEW_CONFIGURATION_VIEW });
  checkLog(xrEnumerateViewConfigurationViews(m_context.m_instance, m_context.m_systemId, m_viewConfig,
...
void XrSystem::recoverEnvironmentBlendModes() {
  ...
  checkLog(xrEnumerateEnvironmentBlendModes(m_context.m_instance, m_context.m_systemId, m_viewConfig, 0, &environmentBlendModeCount, nullptr),
           "Failed to get environment blend mode count",
           m_context.m_instance);
  m_environmentBlendModes.resize(environmentBlendModeCount);
  checkLog(xrEnumerateEnvironmentBlendModes(m_context.m_instance, m_context.m_systemId, m_viewConfig,
