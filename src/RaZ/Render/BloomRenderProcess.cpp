...
BloomRenderProcess::BloomRenderProcess(RenderGraph& renderGraph, unsigned int frameWidth, unsigned int frameHeight) : RenderProcess(renderGraph) {
  ...
  const auto thresholdBuffer = Texture2D::create(frameWidth, frameHeight, TextureColorspace::RGB, TextureDataType::FLOAT);
  ...
  for (std::size_t downscalePassIndex = 0; downscalePassIndex < passCount; ++downscalePassIndex) {
    ...
    const auto downscaledBuffer = Texture2D::create(bufferWidth, bufferHeight, TextureColorspace::RGB, TextureDataType::FLOAT);
  ...
  for (std::size_t upscalePassIndex = 0; upscalePassIndex < passCount - 1; ++upscalePassIndex) {
    ...
    const auto upscaledBuffer = Texture2D::create(bufferWidth, bufferHeight, TextureColorspace::RGB, TextureDataType::FLOAT);
