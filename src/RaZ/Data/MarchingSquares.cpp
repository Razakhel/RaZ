...
uint8_t computeCellConfiguration(const Grid2b& grid, std::size_t widthIndex, std::size_t heightIndex) {
  ...
  return static_cast<uint8_t>(grid.getValue(widthIndex,     heightIndex    ))        // Bottom-left
      | (static_cast<uint8_t>(grid.getValue(widthIndex + 1, heightIndex    )) << 1)  // Bottom-right
      | (static_cast<uint8_t>(grid.getValue(widthIndex + 1, heightIndex + 1)) << 2)  // Top-right
      | (static_cast<uint8_t>(grid.getValue(widthIndex,     heightIndex + 1)) << 3); // Top-left
}
