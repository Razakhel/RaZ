...
TEST_CASE("LuaRender RenderProcess") {
  CHECK(Raz::LuaWrapper::execute(R"(
    ...
    local reinhardToneMapping = ReinhardToneMapping.new(renderGraph)
    reinhardToneMapping:setState(true)
    reinhardToneMapping:enable()
    reinhardToneMapping:disable()
    assert(not reinhardToneMapping:isEnabled())
    reinhardToneMapping:addParent(renderPass)
    reinhardToneMapping:addParent(ReinhardToneMapping.new(renderGraph))
    reinhardToneMapping:addChild(renderPass)
    reinhardToneMapping:addChild(ReinhardToneMapping.new(renderGraph))
    reinhardToneMapping:resizeBuffers(1, 1)
    assert(reinhardToneMapping:recoverElapsedTime() >= 0)
    reinhardToneMapping:setInputBuffer(Texture2D.create(TextureColorspace.RGB))
    reinhardToneMapping:setOutputBuffer(Texture2D.create(TextureColorspace.GRAY))
    reinhardToneMapping:setMaxWhiteValue(0)
