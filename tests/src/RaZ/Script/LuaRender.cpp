...
TEST_CASE("LuaRender RenderGraph") {
  CHECK(Raz::LuaWrapper::execute(R"(
    ...
    assert(renderGraph:addSsrRenderProcess() ~= nil)
...
TEST_CASE("LuaRender RenderProcess") {
  CHECK(Raz::LuaWrapper::execute(R"(
    ...
    local ssaoRenderProcess = SsaoRenderProcess.new(renderGraph)
    ssaoRenderProcess:setState(true)
    ssaoRenderProcess:enable()
    ssaoRenderProcess:disable()
    assert(not ssaoRenderProcess:isEnabled())
    ssaoRenderProcess:addParent(renderPass)
    ssaoRenderProcess:addParent(SsaoRenderProcess.new(renderGraph))
    ssaoRenderProcess:addChild(renderPass)
    ssaoRenderProcess:addChild(SsaoRenderProcess.new(renderGraph))
    ssaoRenderProcess:resizeBuffers(1, 1)
    assert(ssaoRenderProcess:recoverElapsedTime() >= 0)
    ssaoRenderProcess:setInputDepthBuffer(Texture2D.create(TextureColorspace.DEPTH))
    ssaoRenderProcess:setInputNormalBuffer(Texture2D.create(TextureColorspace.RGB))
    ssaoRenderProcess:setOutputBuffer(Texture2D.create(TextureColorspace.GRAY))

    local ssrRenderProcess = SsrRenderProcess.new(renderGraph)
    ...
    ssrRenderProcess:addParent(SsrRenderProcess.new(renderGraph))
    ssrRenderProcess:addChild(renderPass)
    ssrRenderProcess:addChild(SsrRenderProcess.new(renderGraph))
