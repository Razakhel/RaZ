...
TEST_CASE("Listener data", "[audio]") {
  ...
  CHECK(listener.recoverForwardOrientation() == Raz::Axis::Forward);
  CHECK(listener.recoverUpOrientation() == Raz::Axis::Up);

  constexpr float gain = 0.25f;
  constexpr Raz::Vec3f position(1.f, 2.f, 3.f);
  constexpr Raz::Vec3f velocity(3.f, 2.f, 1.f);
  constexpr Raz::Vec3f forward = Raz::Axis::Up;
  constexpr Raz::Vec3f up = Raz::Axis::Left;
