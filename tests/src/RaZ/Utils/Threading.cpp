...
void checkParallelIncrementation(const std::invocable<std::vector<int>&, unsigned int> auto& incrementParallel) {
  ...
  for (unsigned int threadCount = 1; threadCount < 8; ++threadCount) {
    for (std::size_t valueCount = 1; valueCount < maxValueCount; ++valueCount) {
      ...
      CHECK(std::ranges::all_of(values.begin(), values.end(), [] (int val) noexcept { return val == 1; }));
    }
  }
}
...
TEST_CASE("Threading parallelization index", "[utils]") {
  ...
  CHECK_THROWS(Raz::Threading::parallelize(0, 1, [] (const auto&) noexcept {}, threadPool, 0)); // Given 0 as thread count
  CHECK_THROWS(Raz::Threading::parallelize(0, 0, [] (const auto&) noexcept {}, threadPool, 1)); // Zero range
  CHECK_THROWS(Raz::Threading::parallelize(1, 0, [] (const auto&) noexcept {}, threadPool, 1)); // Negative range

  checkParallelIncrementation([&threadPool] (std::vector<int>& values, unsigned int threadCount) {
...
TEST_CASE("Threading parallelization iterator", "[utils]") {
...

  {
    ...
    CHECK_THROWS(Raz::Threading::parallelize(dummy.begin(), dummy.end(), [](const auto&) noexcept {}, threadPool, 0)); // Given 0 as thread count
    CHECK_THROWS(Raz::Threading::parallelize(dummy.begin(), dummy.begin(), [](const auto&) noexcept {}, threadPool, 1)); // Zero range
    CHECK_THROWS(Raz::Threading::parallelize(dummy.end(), dummy.begin(), [](const auto&) noexcept {}, threadPool, 1)); // Negative range
  }
